	// ==UserScript==
// @name cardgames.io Accessibility Fixes
// @description    Improves the accessibility of cardgames.io
// @author         ajackpot
// @copyright 2019-2022 Mozilla Corporation, Derek Riemer
// @license Mozilla Public License version 2.0
// @version        2019.1
// @include https://cardgames.io/reversi/*
// ==/UserScript==
...
// applied.
const DYNAMIC_TWEAK_ATTRIBS = ['data-square', 'class'];

// Tweaks that must be applied whenever an element is added/changed.
const DYNAMIC_TWEAKS = [
	{selector: '#reversi-board-inner',
	...
	{selector: '.table>.black',
		tweak: e => {
			let c = e.getAttribute('class').replace('  ', ' ');
			let cell = document.querySelector('div[data-square*="' + c[7] + '-' + c[10] + '"]');
			if (cell != null) {
				cell.setAttribute('aria-label', '흑');
				if (cell.hasAttribute('data-axS-empty-cell')) {
					cell.removeAttribute('data-axS-empty-cell');
					announce('흑 ' + cell.getAttribute('aria-description') + '에 착수함', 'axS-announce-new-stone');
				}
			}
		},
	},
	{selector: '.table>.white',
		tweak: e => {
			let c = e.getAttribute('class').replace('  ', ' ');
			let cell = document.querySelector('div[data-square*="' + c[7] + '-' + c[10] + '"]');
			if (cell != null) {
				cell.setAttribute('aria-label', '백');
				if (cell.hasAttribute('data-axS-empty-cell')) {
					cell.removeAttribute('data-axS-empty-cell');
					announce('백 ' + cell.getAttribute('aria-description') + '에 착수함', 'axS-announce-new-stone');
				}
			}
		},
	},
