	// ==UserScript==
// @name othello-rust.web.app Accessibility Fixes
// @description    Improves the accessibility of othello-rust.web.app
// @author         ajackpot
// @copyright 2019-2022 Mozilla Corporation, Derek Riemer
// @license Mozilla Public License version 2.0
// @version        2019.1
// @include https://othello-rust.web.app/*
// ==/UserScript==
...
// applied.
const DYNAMIC_TWEAK_ATTRIBS = ['style', 'class', 'tabindex'];
	...
	{selector: '#board',
		tweak: e => {
			for (let i = 0, l = e.children.length; i < l; i++) {
				let x = String.fromCharCode((i % 8) + 97);
				let y = Math.floor(i / 8) + 1;
				e.children[i].setAttribute('aria-description', x + y);
				e.children[i].setAttribute('data-axS-empty-cell', 'true');
			}
		},
	},
	{selector: '#board>button[class="cell"]',
		tweak: e => {
			e.setAttribute('aria-label', '빈칸');
			if (!e.hasAttribute('data-axS-empty-cell')) e.setAttribute('data-axS-empty-cell', 'true');
		},
	},
	{selector: '#board>button[tabindex="0"]',
		tweak: e => {
			e.setAttribute('aria-label', '착수 가능');
		},
	},
	{selector: '#board>.black',
		tweak: e => {
