	// ==UserScript==
// @name wasm-othello.t-yng.jp Accessibility Fixes
// @description    Improves the accessibility of wasm-othello.t-yng.jp
// @author         ajackpot
// @copyright 2019-2022 Mozilla Corporation, Derek Riemer
// @license Mozilla Public License version 2.0
// @version        2019.1
// @include https://wasm-othello.t-yng.jp/*
// ==/UserScript==
...
const DYNAMIC_TWEAKS = [
	...
	{selector: '.e13cimdl3>div[style*="transform: rotateY(0deg)"]',
		tweak: inner => {
			let e = inner.parentNode;
			e.setAttribute('role', 'img');
			e.setAttribute('aria-label', '흑');
			if (!e.parentNode.hasAttribute('data-axS-placed-cell')) announce('흑 ' + e.parentNode.getAttribute('aria-label') + '에 착수함', 'axS-announce-new-stone');
			e.parentNode.setAttribute('data-axS-placed-cell', 'true');
		},
	},
	{selector: '.e13cimdl3>div[style*="transform: rotateY(180deg)"]',
		tweak: inner => {
			let e = inner.parentNode;
			e.setAttribute('role', 'img');
			e.setAttribute('aria-label', '백');
			if (!e.parentNode.hasAttribute('data-axS-placed-cell')) announce('백 ' + e.parentNode.getAttribute('aria-label') + '에 착수함', 'axS-announce-new-stone');
			e.parentNode.setAttribute('data-axS-placed-cell', 'true');
		},
	},
];
